...
try {
  ...
  if ($keyword <> "" ) {
    $sql = "SELECT * FROM tbl_contacts WHERE 1 AND "
            . " (Vertical LIKE :keyword) ORDER BY Vertical ";
    $stmt = $DB->prepare($sql);
    
    $stmt->bindValue(":keyword", $keyword."%");
    
  } else {
    $sql = "SELECT * FROM tbl_contacts WHERE 1 ORDER BY Vertical ";
    $stmt = $DB->prepare($sql);
  }
        ...
        <div class="col-lg-6 pull-left"style="padding-left: 0;"  >
          <span class="pull-left">  
            <label class="col-lg-12 control-label" for="keyword" style="padding-right: 0;">
              <input type="text" value="<?php echo $_GET["keyword"]; ?>" placeholder="Search by Vertical" id="" class="form-control" name="keyword" style="height: 41px;">
            </label>
            </span>
          <button class="btn btn-info">search</button>
        </div>
        </form>
        <div class="pull-right" ><a href="contacts.php"><button class="btn btn-success"><span class="glyphicon glyphicon-user"></span> Add New Entry</button></a></div>
      </div>
...
<?php if (count($results) > 0) { ?>
  ...
  <?php foreach ($results as $res) { ?>
                <tr>
                  <td><?php echo $res["Vertical"]; ?></td>
                  <td><?php echo $res["Client"]; ?></td>
				  <td><?php echo $res["Conclusion"]; ?></td>
                  <td><?php echo $res["email"];echo "  |  "; echo $res["email2"]; echo "  |  "; echo $res["email3"]; echo "  |  "; echo $res["email4"]; ?></td>
				  <td><?php echo $res["Date"];echo "  |  "; echo $res["Time"]; ?></td>
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
          ...
          <?php } else { ?>
        <div class="well well-lg">No entries found.</div>
<?php } ?>
    </div>
  </div>
</div>
